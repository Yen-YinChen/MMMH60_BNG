...
//$where = "WHERE member1_sid= $m_sid OR member2_sid=$m_sid";

$t_sql = "SELECT COUNT(1) FROM friends WHERE member1_sid= $m_sid OR member2_sid=$m_sid";
$totalRows = $pdo->query($t_sql)->fetch(PDO::FETCH_NUM)[0];

//----if the user has no friends at the moment, set null and jump to page
if ($totalRows == 0) {
    $rows = false;
    $friendsRows = false;
    $fsArray = false;
    ...
    $p_sql = sprintf("SELECT * FROM friends WHERE member1_sid= $m_sid OR member2_sid=$m_sid ORDER BY created_at DESC LIMIT %s, %s", ($page-1)*$perPage, $perPage);

    $rows = $pdo->query($p_sql)->fetchAll();

//-------Collect friends' sid in an array--------
    $fsArray = array();

    foreach ($rows as $r){
        if ($r['member1_sid']!= $m_sid) $fsArray[] = $r['member1_sid'];
        if ($r['member2_sid']!= $m_sid) $fsArray[] = $r['member2_sid'];
    }

    /*Query into `member` table and fetchAll the rows of member's friends*/
    $fsArrayStr = implode(",", $fsArray);
    $fs_sql = sprintf("SELECT * FROM members WHERE member_sid IN (%s)", $fsArrayStr);
    $friendsRows = $pdo->query($fs_sql)->fetchAll();
    $friendsArray2 = [];
    foreach ($friendsRows as $r){
        $friendsArray2[$r['member_sid']] = $r;
    }


//echo json_encode($friendsRows, JSON_UNESCAPED_UNICODE);
//echo json_encode(count($friendsRows), JSON_UNESCAPED_UNICODE);

    /* -----Defining friend's profile picture:  --- */
    /*
        $defaultPicName1 = $r['member_id'] . '.jpg';
        $defaultPicName2 = $r['member_id'] . '_1.jpg';
        $defaultPicName3 = $r['member_id'] . '_2.jpg';
        $profilePicture1 = isset($r['profile_pic01']) ? $r['profile_pic01'] : $defaultPicName1;
        $profilePicture2 = isset($r['profile_pic02']) ? $r['profile_pic02'] : $defaultPicName2;
        $profilePicture3 = isset($r['profile_pic03']) ? $r['profile_pic03'] : $defaultPicName3;
    */
    ...
    <div class="container member-wrapper ycn">
        ...
        <div class="container friendlist-contain-ycn">
            <div class="row friendlist-wrapper">
                ...
                <?php else: ?>
                <?php endif; ?>
                <?php if(!empty($friendsRows)): ?>
                <?php foreach($fsArray as $fid):
                    $r = $friendsArray2[$fid];
                    ?>
                    <div class="friendlist-card-div col-6 col-xl-3 ycn" data-sid="<?php echo $r['member_sid']; ?>">
                        <div class="friendlist-card-pic ycn">
                        ...
                        <div class="friendlist-card-message ycn">
                            <a href="#" class="friendlist-message-a">
                                <svg class="message-icon-svg ycn" xmlns="http://www.w3.org/2000/svg"
                                 xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 26 21">
                                    <defs>
                                        <clipPath id="clip-path-message">
                                            <path id="Shape"
                                              d="M23.4,21H2.6A2.615,2.615,0,0,1,0,18.375L.013,2.625A2.609,2.609,0,0,1,2.6,0H23.4A2.615,2.615,0,0,1,26,2.625V18.375A2.615,2.615,0,0,1,23.4,21ZM2.6,2.625V5.249L13,11.812,23.4,5.249V2.625L13,9.188Z"
                                              transform="translate(-0.035 -0.359)" fill="#036" />
                                        </clipPath>
                                    </defs>
                                    <g id="Group_566" data-name="Group 566" transform="translate(0.035 0.359)"
                                   opacity="0.28">
                                        <path id="Shape-2" data-name="Shape"
                                          d="M23.4,21H2.6A2.615,2.615,0,0,1,0,18.375L.013,2.625A2.609,2.609,0,0,1,2.6,0H23.4A2.615,2.615,0,0,1,26,2.625V18.375A2.615,2.615,0,0,1,23.4,21ZM2.6,2.625V5.249L13,11.812,23.4,5.249V2.625L13,9.188Z"
                                          transform="translate(-0.035 -0.359)" fill="#b7c5d3" />
                                        <g id="Mask_Group_65" data-name="Mask Group 65" transform="translate(0 0)"
                                       clip-path="url(#clip-path-message)">
                                            <g id="Icon_Fill" data-name="Icon Fill"
                                           transform="translate(-2.624 -5.249)">
                                                <rect id="BG" width="32" height="32" transform="translate(-0.411 -0.11)"
                                                  fill="#036" />
                                            </g>
                                        </g>
                                    </g>
                        ...
                        <div class="member-buttons-div-ycn" style="margin-top: 2rem;">
                            <div class="link-to-social-div ycn">
                                <button id="social" class="btn btn-primary">
                                    <a href="<?php echo WEB_ROOT ?>/page1_YK.php">前往交友首頁</a>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <?php if (isset($_SESSION['user'])): ?>
            <div class="member-buttons-div-ycn">
                <div class="link-to-social-div ycn">
                    <button id="social" class="btn btn-primary">
                        <a href="<?php echo WEB_ROOT ?>/member_friendReqsList_YCN.php">查看好友邀請</a>
                    </button>
                </div>
            </div>
        <?php else: ?>
...
<script>
    ...
    $('.friendlist-card-div').click(function () {
        ...
        $.ajax({
            url: 'member_friendList_show-api_YCN.php',
            method: 'GET',
            datatype: 'json',
            data:{
                showFriendSid: currentSid
            }
        }).done(function(data){
            fData = data;
            frData = JSON.parse(fData)
            console.log(frData.fRow);
            renderLightbox();
    ...
    function renderLightbox(){
        $('.lightbox').html('');
        if (frData.fRow){
            $('.lightbox').append(lightboxTpl(frData.fRow));
            $('.lightbox').show();
        }
    }
</script>
